...
//	while(1);
    ...
    for (int i = 0; i < num_cards && i < MAX_NUM_CARDS; i++) {
      EEPROM_ReadBuffer(FIRST_BLOCK_ADDR + 272*i, nicknames[i], 16);
    }

    uint16_t next_card_addr = FIRST_BLOCK_ADDR + num_cards*272;
...
//		HAL_Delay(100);
    ...
    switch (state) {
        ...
        case AddCard: {
          ...
          next_card_addr = FIRST_BLOCK_ADDR + num_cards*272;
        ...
        case ViewCard: {
            ...
            uint16_t block_addr = FIRST_BLOCK_ADDR + 272*card_choice;
        ...
        case EditNickname: {
          ...
          EEPROM_WriteBuffer(FIRST_BLOCK_ADDR + card_choice*272, new_nickname, NICKNAME_LEN);
        ...
        case EditTrack1: {
          ...
          EEPROM_WriteBuffer(FIRST_BLOCK_ADDR + card_choice*272 + NICKNAME_LEN, new_track1, TRACK1_LEN);
        ...
        case EditTrack2: {
          ...
          EEPROM_WriteBuffer(FIRST_BLOCK_ADDR + card_choice*272 + NICKNAME_LEN + TRACK1_LEN, new_track2, TRACK2_LEN);
...
void delete_card(int card) {
  ...
  if (card == updated_num_cards - 1) {
	  ...
	  for (int i = card; i < updated_num_cards - 1; i++) {
		  ...
		  uint16_t card_addr_to_replace = FIRST_BLOCK_ADDR + i*272;
		  uint16_t card_next_addr = card_addr_to_replace + 272;
